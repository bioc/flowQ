...
<<transformBoundaryData, echo=true,results=hide>=
...
<<Data Normalization, echo=true,results=hide>>=
  ...
  nData<-normalizeSets(flowList=boundData,dupes=dupes[3])
@
...
<<Density plots, echo=true,results=hide>>=
#resDensity <- qaProcess.DensityPlot(nData,dyes=NULL,outdir="myDir",alpha=0.2)
resDensity <- qaProcess.DensityPlot(nData,dyes=c("FSC-A","CD8"),outdir="myDir",alpha=0.2,pdf=TRUE)
imagePath<-resDensity@summaryGraph@fileNames[1]
densityUrl<-writeQAReport(nData[[1]], list(resDensity), outdir="myDir")
browseURL(densityUrl)
@

<<produceDensityimage,echo=false,results=tex>>=
