...
you could build arbitraily complex procedures, potentially comparing
...
sophisticated version, already a part of the package in the function
(qaProcess.cellnumber). The idea is to take a single \Rclass{flowSet}
as input and check whether the total number of cells for the
individual \Rclass{flowFrames} is above a certain threshold. The user
should be able to specify the \Rclass{flowSet}, the threshold value,
the output directory in which to generate the images and the name of
the process which will later appear in the report. We could use the
...
qaProcess constructor in form of a list, so we first create a list of
appropriate length. We then iterate over each \Rclass{flowFrame}
in order to check whether the number of cells is above the threshold
and create the appropriate \Rclass{numericAggregator} objects. In a
final step we provide both the \Rclass{qaGraph} object containing the
...
such things in density plots of the data. If we try to translate this
back into the abstract \Rpackage{flowQ} framework, we realize that we
will need one single \Rclass{qaGraph} object for each subprocess and
for each frame. We don't really have a need for a per-frame summary
graph, so we can skip this step. There might be other situations where
it makes sense to have all parameter-specific plots, frame-specific
summary plots and also overall summary plots. We will first take care
...
mainly exists for internal software design purpose. We fill exactly
the same way we would fill a regular R list.
...
but other than that they are pretty similar to the examples shown
before.
...
details of such extensions are beyond the scope o this document, but
