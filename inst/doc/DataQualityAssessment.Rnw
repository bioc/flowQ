...
<<ECDF plots 1, echo=true,results=hide>>=
resFSCECDF <- qaProcess.ECDFPlot(nData,dyes=c("FSC-A","SSC-A"),outdir="myDir",alpha=0.28,pdf=TRUE)
imagePath<-resFSCECDF@summaryGraph@fileNames[1]
#ecdfUrl<-writeQAReport(nData[[1]], list(resECDF), outdir="myDir",pdf=TRUE)
#browseURL(ecdfUrl)
@
...
resCD8ECDF <- qaProcess.ECDFPlot(nData,dyes=c("CD8","CD27"),outdir="myDir",alpha=0.28,pdf=TRUE)
...
information is left empty. From the ECDF plots it appears that the forward/side 
scatter as well as the CD8 intensities for patient "pid02050" appears to be different
from the rest of the group indicating some problem with the experimental or data
collection procedure.However, the intensity for dye CD27 for patient "pid02050" 
appears similar to the rest of the group.
...
"pid02050". A difference in the shape of the distribution for patient "pid02050"
can be observed from the CD8 density plot. 
