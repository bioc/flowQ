...
\myincfig{flowQStructure}{\textwidth}{Schematics of a
...
distances were then summed up and then normalized by the maximum value of the sum 
of the KL distances obtained during the QA process for each floursecence
parameter. This yields a measure that could be useful for better comparison
 of the magnitude of differences. Univariate outlier detection was performed
on the normalized distance values to identify patient panels where ecdf 
...
values for each aliquot. The sum of the pairwise distances for each patient 
were then normalized to the maximum value of the sum of the distances for 
each parameter that was calculated to yield a number between 0 and 1. 
Univariate outlier detection was performed on the normalized distance values
to identify patient panels where density plots are different from the rest
of the group.
